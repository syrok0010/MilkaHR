...
export class RelevantJobsRingChartComponent {
  ...
  count = computed(() => {
    const i = this.index();
    if (Number.isNaN(i)) {
      return this.jobsByPriority().map(x => x.opened).reduce((s, a) => s + a, 0);
    }
    return this.jobsByPriority()[i].opened;
  });
  values = computed(() => this.jobsByPriority().map(e => e.opened));
  labels: string[] = Object.values(PriorityLevel).map((level) => PriorityLevel[level]);
  jobsByPriority = signal<StatisticByPriority[]>([new StatisticByPriority({ opened: 10, all: 20, level: PriorityLevel.High}), new StatisticByPriority({opened: 15, all: 30, level: PriorityLevel.Medium})])
